...
- **Desktop environment rule:** For any feature involving file downloads, file saves, clipboard, OS dialogs, or native system access: always add an explicit acceptance criterion that the feature works in the **Tauri desktop app** (not just in a web browser). Include a technical note that the developer must use `window.__TAURI__` detection with `tauri-plugin-dialog`/`tauri-plugin-fs` for the desktop path. This prevents environment-specific regressions from slipping past the pipeline (e.g. PDF export working in browser but silently failing in WebView2).
